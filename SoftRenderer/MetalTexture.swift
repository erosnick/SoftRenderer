...
class MetalTexture: NSObject {
  ...
  func loadTexture(device: MTLDevice, commandQueue: MTLCommandQueue, flip: Bool){
    ...
    let imageData: NSData = NSData(contentsOfFile: path)!
    
    let image: CGImageRef = (NSBitmapImageRep(data: imageData)?.CGImage)!
    ...
    if (isMipmaped == true){
      generateMipMapLayersUsingSystemFunc(texture, device: device, commandQueue: commandQueue, block: { (buffer) -> Void in
  ...
  func image(mipLevel: Int) -> CGImageRef{
...
//    var image = UIImage(CGImage: imgRef)
  ...
  func generateMipMapLayersUsingSystemFunc(texture: MTLTexture, device: MTLDevice, commandQueue: MTLCommandQueue,block: MTLCommandBufferHandler){
